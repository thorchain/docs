...
5. **Remote full nodes** - for every chain that is supported by the network, each THORNode operator will run their own full node of each chain \(Bitcoin, Ethereum, Binance, etc\).
